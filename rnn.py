...
class NER_net:
    ...
    def _build_net(self):
        ...
        source = self.iterator.source #[3,5,7,9,...] #一句话中各个单词对应的索引
        ...
        outputs = tf.concat([forward_out, backward_out], axis=2) #将输入输出进行映射
        ...
        projection = tf.matmul(x_reshape, W) + b   #[n, tags_num]

        # -1 to time step
        self.outputs = tf.reshape(projection, [self.batch_size, -1, TAGS_NUM])

        self.seq_length = tf.convert_to_tensor(self.batch_size * [max_sequence_in_batch], dtype=tf.int32)#[100,100,100.....]，表示每个序列的长度
...
def predict(net, tag_table, sess):
    ...
    f = open("resource/predict_result.txt","a")
    while True:
        ...
        viterbi_sequence, _ = tf.contrib.crf.viterbi_decode(
            tf_unary_scores, tf_transition_params)  #返回最高分数的索引标签，这里用标签的序号表示
        tags = ""
...
if __name__ == '__main__':
    ...
    action = config.FLAGS.action #当前的行为
    # 获取词的总数。
    vocab_size = get_src_vocab_size()  #训练数据中不重复的词的个数
    ...
    src_vocab_table, tgt_vocab_table = create_vocab_tables(src_vocab_file, tgt_vocab_file, src_unknown_id,
                                                           tgt_unknown_id)  #根据词获得索引
    iterator = None
    embedding = load_word2vec_embedding(vocab_size) #返回embedding层，（vocab_size+2, 300）

    if action == 'train':
        iterator = get_iterator(src_vocab_table, tgt_vocab_table, vocab_size, BATCH_SIZE) # BatchedInput对象
        print(iterator.source)
    elif action == 'predict':
        BATCH_SIZE = 1
        DROPOUT_RATE = 1.0
    ...
    tag_table = tag_to_id_table() #索引——》单词

    net = NER_net("ner", iterator, embedding, BATCH_SIZE) #传入当前的iterator，构造net
