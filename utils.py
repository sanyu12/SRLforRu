...
def get_iterator(src_vocab_table, tgt_vocab_table, vocab_size, batch_size, buffer_size=None, random_seed=None,
    ...
    src_dataset = tf.data.TextLineDataset(src_file) #dataset中的每一个元素对应文件中的一行，可以用来读取csv文件
    tgt_dataset = tf.data.TextLineDataset(tgt_file)
    #[src_dataset] + [tgt_dataset] - --> [src_dataset, tgt_dataset]
    src_tgt_dataset = tf.data.Dataset.zip((src_dataset, tgt_dataset))

    src_tgt_dataset = src_tgt_dataset.shuffle(
        buffer_size, random_seed)
    #进行分词处理[（[],[]）.....]
    # 张量变化举例 ['上海　浦东', 'LOC1  LOC2'] ---> [['上海', '浦东'], ['LOC1', 'LOC2']]
    ...
    #张量变化举例  [[1, 2], [1, 2]] ---> [[1, 2], [1, 2],[src_size], [tgt_size]]
    src_tgt_dataset = src_tgt_dataset.map(
        lambda src, tgt_in: (
  ...
  #数据对齐处理
    ...
    def key_func(unused_1, unused_2, src_len, tgt_len):
        ...
        """
        if src_max_len:
            bucket_width = (src_max_len + num_buckets - 1) // num_buckets  #桶的宽度
        else:
            bucket_width = 10

        bucket_id = tf.maximum(src_len // bucket_width, tgt_len // bucket_width) #数据的长度所对应的桶的id号
        #返回数据所在桶的索引
        return tf.to_int64(tf.minimum(num_buckets, bucket_id)) #如果桶的id号大于桶的最大编号，那么就把数据全部放入最后一个桶中
...
def load_word2vec_embedding(vocab_size):
    ...
    for index, line in enumerate(f):
        values = line.split()
        try:
            coefs = np.asarray(values[1:], dtype='float32')  # 取向量 ,type : ndarray
            # print(type(coefs))
        except ValueError:
